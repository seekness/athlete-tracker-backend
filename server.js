...
const checkRole = (role) => {
  return (req, res, next) => {
    if (req.user && req.user.role === role) {
      ...
      res
        .status(403)
        .json({ message: "Pristup zabranjen: Nemate potrebne privilegije." });
    }
  };
};
...
//     if (role === "sportista") {
//       const [athlete] = await connection.query(
...
//           .status(404)
//           .send(
//             "Sportista sa unetim korisničkim imenom nije pronađen. Trener mora prvo uneti sportistu."
//           );
//       }
...
//         return res
//           .status(409)
//           .send("Ovaj sportista je već povezan sa nalogom.");
//       }
//     } else {
...
//   try {
//     const [existingLocation] = await dbPool.query(
...
//       "SELECT * FROM `groups` ORDER BY naziv ASC"
//     );
//     res.json(rows);
//   } catch (error) {
...
//   }
//   try {
//     await dbPool.query("UPDATE `groups` SET naziv = ?, opis = ? WHERE id = ?", [
//       naziv,
//       opis,
//       id,
//     ]);
...
//     await connection.query("DELETE FROM group_memberships WHERE group_id = ?", [
//       id,
//     ]);
//     // Brisanje same grupe
...
//--- RUTAS ZA VEŽBE I KATEGORIJE VEŽBI ---
...
//       .status(400)
//       .send(
//         "Naziv vežbe, mišićna grupa, kategorija i vrsta unosa su obavezni."
//       );
//   }
//   try {
//     const [result] = await dbPool.query(
...
//       korisnik_id
//     ]);
//     res.status(201).send("Trener je uspešno dodat.");
//   } catch (error) {
...
//   try {
//     const [trainers] = await dbPool.query(
//       `
//             SELECT * FROM trainers order by prezime ASC
//             `
//     );
...
//     res
//       .status(500)
//       .json({ message: "Greška pri dobijanju podataka o treneru." });
//   }
// });
...
//       return res
//         .status(404)
//         .json({ message: "Trener nije pronađen za ažuriranje." });
//     }
...
//     res
//       .status(500)
//       .json({ message: "Greška pri ažuriranju podataka o treneru." });
//   }
// });
...
//     return res
//       .status(403)
//       .json({ message: "Nemate dozvolu za brisanje ovog profila." });
//   }
...
//       return res
//         .status(404)
//         .json({ message: "Trener nije pronađen za brisanje." });
//     }
...
// });
  ...
  //   try {
  //     const [rows] = await dbPool.query(
  //       "SELECT * FROM muscle_groups ORDER BY naziv ASC"
  //     );
  //     res.json(rows);
  //   } catch (error) {
  ...
  // // POST /api/admin/membership/fees
// app.post("/api/admin/membership/fees",
//   authenticateToken,
//   checkRole("admin"),
//   async (req, res) => {
...
//         [amount_first, amount_second, amount_third, valid_from]
//       );
//       res
//         .status(201)
//         .json({
//           message: "Nova visina članarine uspešno definisana.",
...
// // GET /api/membership/fees - Ruta za dobijanje svih članarina (za trenere i admine)
// app.get("/api/membership/fees", authenticateToken, async (req, res) => {
//   try {
...
// app.put("/api/admin/membership/fees/:id",
//   authenticateToken,
//   checkRole("admin"),
//   async (req, res) => {
//     const { id } = req.params;
...
//         [amount_first, amount_second, amount_third, valid_from, id]
//       );
...
// // DELETE /api/admin/membership/fees/:id
// app.delete("/api/admin/membership/fees/:id",
//   authenticateToken,
//   checkRole("admin"),
//   async (req, res) => {
//     const { id } = req.params;
...
// // GET /api/membership/athletes
// app.get("/api/membership/athletes", authenticateToken, async (req, res) => {
...
//                     athletes a
//                 JOIN
//                     group_memberships gm ON a.id = gm.athlete_id
//                 JOIN
//                     program_group_assignments pga ON gm.group_id = pga.group_id
//                 WHERE
//                     a.aktivan = 1 AND (? = 'admin' OR 
...
//                     athletes a
//                 JOIN
//                     program_athlete_assignments paa ON a.id = paa.athlete_id
//                 WHERE
//                     a.aktivan = 1 AND (? = 'admin' OR 
...
// // GET /api/membership/payments/monthly
// app.get("/api/membership/payments/monthly",
//   authenticateToken,
//   async (req, res) => {
//     const userId = req.user.id;
//     const userRole = req.user.role;
...
//                 athletes a
//             JOIN
//                 membership_payments mp ON a.id = mp.athlete_id
//             WHERE
//                 -- Filtriranje na osnovu trenera (grupe ili individualni sportisti)
//                 (
//                     ? = 'admin' OR
//                     a.id IN (
...
//         const athleteName = `${current.ime} ${current.prezime}`;
...
//         if (!acc[athleteId].payments[monthKey]) {
//           acc[athleteId].payments[monthKey] = [];
//         }

//         acc[athleteId].payments[monthKey].push({
...
//       // Konvertovanje objekta u niz za lakši prikaz na frontendu
//       const formattedPayments = Object.values(monthlyPayments);
...
// app.post("/api/membership/payments", authenticateToken, async (req, res) => {
//   const { athlete_id, amount_paid, child_order, note, payment_month } =
//     req.body;
//   try {
//     const [result] = await dbPool.query(
//       `INSERT INTO membership_payments (athlete_id, payment_date, amount_paid, child_order, note, payment_month) VALUES (?, CURDATE(), ?, ?, ?, ?)`,
//       [athlete_id, amount_paid, child_order, note, payment_month]
//     );
//     res
//       .status(201)
//       .json({
//         message: "Uplata uspešno zabeležena.",
...
// app.get("/api/membership/athletes-status",
//   authenticateToken,
//   async (req, res) => {
//     const userId = req.user.id;
//     const userRole = req.user.role;
...
//     } catch (error) {
//       console.error(
//         "Greška pri dobijanju liste sportista za članarinu:",
//         error
//       );
//       res.status(500).json({ message: "Greška na serveru." });
//     }
...
// Ruta za dodavanje novog sportiste
...
app.get("/api/athletes/:athleteId/groups",
  authenticateToken,
  async (req, res) => {
    ...
    `;
      ...
      if (groups.length === 0) {
        ...
        const athleteCheck = await dbPool.query(
...
app.get("/api/groups/:groupId/athletes",
  authenticateToken,
  async (req, res) => {
...
app.post("/api/groups/:groupId/athletes",
  authenticateToken,
  async (req, res) => {
    const { groupId } = req.params;
...
app.put("/api/athletes/:athleteId", authenticateToken, async (req, res) => {
  ...
  try {
    ...
    if (group_ids && group_ids.length > 0) {
      const insertGroupValues = group_ids.map((groupId) => [
        athleteId,
        groupId,
      ]);
...
// app.get("/api/programs/:programId/trainings",
//   authenticateToken,
//   isTrener,
//   async (req, res) => {
...
// app.post("/api/programs/:programId/trainings",
//   authenticateToken,
//   isTrener,
//   async (req, res) => {
//     const { programId } = req.params;
...
app.put("/api/trainings/:trainingId",
  authenticateToken,
  isTrener,
  async (req, res) => {
...
app.delete("/api/trainings/:trainingId",
  authenticateToken,
  isTrener,
  async (req, res) => {
    const { trainingId } = req.params;
...
app.post("/api/assign-program/group", authenticateToken, async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ message: "Greška pri dodeljivanju programa grupi." });
  }
});
...
app.post("/api/assign-program/athlete", authenticateToken, async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ message: "Greška pri dodeljivanju programa sportisti." });
  }
});
...
app.put("/api/assign-program/group", authenticateToken, async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ message: "Dodela programa grupi uspešno ažurirana." });
  } catch (error) {
    console.error(error);
    res
      .status(500)
      .json({ message: "Greška pri ažuriranju dodele programa grupi." });
  }
});
...
app.put("/api/assign-program/athlete", authenticateToken, async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ message: "Dodela programa sportisti uspešno ažurirana." });
  } catch (error) {
    console.error(error);
    res
      .status(500)
      .json({ message: "Greška pri ažuriranju dodele programa sportisti." });
  }
});

// DELETE ruta za brisanje dodele programa grupi
app.delete("/api/assign-program/group/:id",
  authenticateToken,
  async (req, res) => {
    const { id } = req.params;
    const userId = req.user.id;
    ...
    try {
      ...
      if (userRole !== "admin" && assignedByUserId !== userId) {
        return res
          .status(403)
          .json({ message: "Nemate dozvolu da obrišete ovu dodelu." });
      }
...
app.delete("/api/assign-program/athlete/:id",
  authenticateToken,
  async (req, res) => {
    const { id } = req.params;
    const userId = req.user.id;
    ...
    try {
      ...
      // Izvrši brisanje
      const deleteQuery =
        "DELETE FROM program_athlete_assignments WHERE id = ?";
      await dbPool.query(deleteQuery, [id]);
...
app.get("/api/assigned-programs/groups",
  authenticateToken,
  async (req, res) => {
    const userRole = req.user.role;
    const userId = req.user.id;
    ...
    try {
      ...
      console.error(
        "Greška pri dobijanju dodeljenih programa za grupe:",
        error
      );
      res.status(500).send("Došlo je do greške na serveru.");
    }
...
app.get("/api/assigned-programs/athletes",
  authenticateToken,
  async (req, res) => {
    const userRole = req.user.role;
    const userId = req.user.id;
    ...
    `;
      ...
      console.error(
        "Greška pri dobijanju dodeljenih programa za sportiste:",
        error
      );
      res.status(500).send("Došlo je do greške na serveru.");
    }
...
// app.get("/api/coaches/:coachId/assigned-athletes",
//   authenticateToken,
//   async (req, res) => {
//     try {
...
//       res
//         .status(500)
//         .json({ message: "Greška pri dobijanju dodeljenih sportista." });
//     }
//   }
// );

// // Ruta za dobijanje dodeljenih grupa za određenog trenera
// app.get("/api/coaches/:coachId/assigned-groups",
//   authenticateToken,
//   async (req, res) => {
//     try {
//       const { coachId } = req.params;
...
//       res
//         .status(500)
//         .json({ message: "Greška pri dobijanju dodeljenih grupa." });
//     }
//   }
// );

// // Ruta za dobijanje dodeljenih sportista za određenog trenera na osnovu njegovog id usera
// app.get("/api/coaches/:userId/assigned-athletes-iduser",
//   authenticateToken,
//   async (req, res) => {
//     try {
//       const { userId } = req.params;
...
// app.get("/api/coaches/:userId/assigned-groups-iduser",
//   authenticateToken,
//   async (req, res) => {
//     try {
//       const { userId } = req.params;
...
//     return res
//       .status(403)
//       .json({ message: "Samo administrator može vršiti dodele." });
//   }
...
//       );
//       const athleteAssignments = athlete_ids.map((athleteId) => [
//         coach_id,
//         athleteId,
//       ]);
//       await connection.query(
...
//     res
//       .status(200)
//       .json({ message: "Sportisti i grupe su uspešno dodeljeni." });
//   } catch (error) {
//     await connection.rollback();
//     console.error("Greška pri dodeli:", error);
//     res
//       .status(500)
//       .json({ message: "Greška pri dodeli. Molimo pokušajte ponovo." });
//   } finally {
//     connection.release();
//   }
// });
...
// app.delete("/api/coach/unassign-athlete/:athleteId",
//   authenticateToken,
//   async (req, res) => {
//     const { athleteId } = req.params;
//     const coachId = req.user.id;
...
//         return res
//           .status(404)
//           .json({ message: "Dodela za datog sportistu nije pronađena." });
//       }

//       res
//         .status(200)
//         .json({ message: "Sportista je uspešno uklonjen sa liste trenera." });
//     } catch (error) {
//       console.error(error);
//       res
//         .status(500)
//         .json({ message: "Greška pri uklanjanju dodele sportisti." });
//     }
//   }
// );

// // DELETE ruta za uklanjanje dodele trenera grupi
// app.delete("/api/coach/unassign-group/:groupId",
//   authenticateToken,
//   async (req, res) => {
//     const { groupId } = req.params;
//     const coachId = req.user.id;
...
//         return res
//           .status(404)
//           .json({ message: "Dodela za datu grupu nije pronađena." });
//       }

//       res
//         .status(200)
//         .json({ message: "Grupa je uspešno uklonjena sa liste trenera." });
//     } catch (error) {
//       console.error(error);
...
app.get("/api/trainings/:id/attendance",
  authenticateToken,
  async (req, res) => {
...
app.post("/api/trainings/:id/attendance",
  authenticateToken,
  async (req, res) => {
    const { id: trainingIdParam } = req.params;
    const trainingId = parseInt(trainingIdParam, 10);
    ...
    try {
      ...
      res
        .status(200)
        .json({ message: "Attendance records saved successfully." });
    } catch (error) {
      if (connection) {
        await connection.rollback();
        connection.release();
